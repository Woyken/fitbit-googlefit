...
class Remote:
    ...
    def SyncFitbitToGoogleFit(self, dataType, date_stamp):
        ...
        if dataType in ('steps', 'distance', 'heart_rate', 'calories'):
            ...
            return self.SyncFitbitSleepV12ToGoogleFit(date_stamp)
        else:
    ...
    def SyncFitbitSleepToGoogleFit(self, date_stamp):
        ...
        """
        data_source_id = self.convertor.GetDataSourceId('sleep')
        date_obj = self.convertor.parseHumanReadableDate(date_stamp)

        # Get sleep data for a given date
        fitbit_sleeps = self.ReadFromFitbit(self.fitbitClient.get_sleep, date_obj)['sleep']

        # Iterate over each sleep log for that day
        sleep_count = 0
        for fit_sleep in fitbit_sleeps:
            ...
            # convert all fitbit data points to google fit data points
            google_points = [self.convertor.ConvertFibitPoint(
                (date_stamp if start_time <= point['dateTime'] else next_date_stamp),
                point, 'sleep') for point in minute_points]

            # 1. Write a fit session about sleep
            google_session = self.convertor.ConvertGFitSleepSession(google_points, fit_sleep['logId'])
            self.WriteSessionToGoogleFit(google_session)

            # 2. create activity segment data points for the activity
            self.WriteToGoogleFit(data_source_id, google_points)

        print("synced sleep - {} logs".format(sleep_count))

    def SyncFitbitSleepV12ToGoogleFit(self, date_stamp):
        """
        Sync sleep data for a given day from Fitbit to Google fit.
        ...
        # Get sleep data for a given date
        fitbit_sleeps = self.ReadFromFitbit(self.fitbitClient.get_sleep_v12, date_obj)['sleep']
    ...
    def SyncFitbitActivitiesToGoogleFit(self, start_date='', callurl=None):
        ...
        if not callurl:
            callurl = '{}/user/-/activities/list.json?afterDate={}&sort=asc&offset=0&limit=20'.format(
                self.FITBIT_API_URL, start_date)
        ...
        if len(startTimeMillis) > 0:
            print("Synced {} exercises between : {} -- {}".format(len(activities),
                                                                  datetime.fromtimestamp(
                                                                      min(startTimeMillis) / 1000).strftime('%Y-%m-%d'),
                                                                  datetime.fromtimestamp(
                                                                      max(endTimeMillis) / 1000).strftime('%Y-%m-%d')))
        else:
